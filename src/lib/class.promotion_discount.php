...
class promotion_discount
{
    ...
    {
        if ($type == PROMOTIONS_DISCOUNT_PRODUCT || $type == PROMOTIONS_DISCOUNT_AMOUNT)
        {
            $this->_type = $type;
        }
    }
    ...
    {
        if (is_null($this->_type) || is_null($this->_desc) || is_null($this->_amt)
            || (floatval($this->_amt) <= 0.0) || is_null($this->_promo_id))
        {
            return FALSE;
        }
