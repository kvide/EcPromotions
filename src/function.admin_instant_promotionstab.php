...
$data = $db->GetArray($query, array(promotion::TYPE_INSTANT));
if (is_array($data))
{
    for ($i = 0; $i < count($data); $i ++)
    {
        $rec = &$data[$i];
        $rec['edit_url'] = $this->create_url($id, 'admin_addpromo', $returnid, array('promoid' => $rec['id']));
        $rec['edit_link'] = $this->CreateImageLink($id, 'admin_addpromo', $returnid, $this->Lang('edit'),
                                                    'icons/system/edit.gif', array('promoid' => $rec['id']));
        $rec['delete_url'] = $this->create_url($id, 'admin_delpromo', $returnid, array('promoid' => $rec['id']));
        $rec['delete_link'] = $this->CreateImageLink($id, 'admin_delpromo', $returnid, $this->Lang('delete'),
                                                    'icons/system/delete.gif', array('promoid' => $rec['id']), '',
...
$smarty->assign('addlink', $this->CreateImageLink($id, 'admin_addpromo', $returnid, $this->Lang('prompt_addpromo'),
                                                    'icons/system/newobject.gif', array('type' => promotion::TYPE_INSTANT),
                                                    '', '', false));
if (is_array($data))
{
...
//
echo $this->ProcessTemplate('admin_instant_promotionstab.tpl');
